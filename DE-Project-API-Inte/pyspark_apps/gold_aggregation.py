...
def create_product_performance_summary(spark, silver_path, gold_path, processing_date):
    ...
    try:
        ...
        print(f" No silver orders found for product performance.")
...
def create_sales_daily_summary(spark, silver_path, gold_path, processing_date):
    ...
    try:
        ...
        print(f" No silver orders found for sales summary.")
    except Exception as e:
        print(f" Error: {e}")
        raise
...
def create_customer_segments(spark, silver_path, gold_path, processing_date):
    ...
    try:
        ...
        print(f" No silver orders found for customer segmentation.")
    except Exception as e:
        print(f" Error: {e}")
        raise
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("\Usage: python gold_aggregation.py <silver_base_path> <gold_base_path> <processing_date YYYY-MM-DD>\n")
        sys.exit(1)
